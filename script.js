...
document.addEventListener('DOMContentLoaded', () => {
        ...
        const lawPromises = Object.values(lawManifest).flatMap(law => 
            law.files.map(fileInfo =>
                fetch(fileInfo.path)
                    ...
                    .catch(err => console.error(`خطا در بارگذاری قانون ${fileInfo.path}:`, err))
            )
        );
        ...
        const guidePromises = Object.values(lawManifest)
            ...
            .map(law => 
                fetch(law.guide_path)
                    ...
                    .catch(err => console.error(`خطا در بارگذاری راهنما ${law.guide_path}:`, err))
            );
    ...
    function renderSearchResults(results, term) {
        document.querySelectorAll('.tab-content').forEach(tc => {
            ...
            const lawKey = tc.id;
            const relevantResults = results.filter(r => r.lawInfo.title === lawManifest[lawKey].title);
    ...
    mainContent.addEventListener('click', function(e) {
        ...
        const articleContentElement = articleLi.querySelector('.article-content');
        if (!articleContentElement) return;

        // استخراج متن خام بدون تگ‌های HTML
        const tempDiv = document.createElement('div');
        tempDiv.innerHTML = articleContentElement.innerHTML.replace(/<br\s*\/?>/gi, '\n');
        const articleContent = tempDiv.textContent || tempDiv.innerText || '';

        const siteUrl = window.location.origin || "kanoon-hoghooghi.ir"; // یک آدرس پیش‌فرض
        const attribution = `\n\nمنبع: کانون حقوقی ایران - ${siteUrl}`;

        if (target.classList.contains('copy-btn')) {
            navigator.clipboard.writeText(articleContent.trim() + attribution).then(() => {
        ...
        if (target.classList.contains('share-btn')) {
            if (navigator.share) {
                navigator.share({
                    ...
                    text: articleContent.trim() + attribution,
